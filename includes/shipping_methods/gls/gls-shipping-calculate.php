...
function GLS_Shipping_Method(): void
{
    if (!class_exists('GLS_Shipping_Method')) {
        class GLS_Shipping_Method extends WC_Shipping_Method
        {
            ...
            {
                $this->id = 'gls';
                $this->method_title = __('GLS Online Shipping', 'gls');
                $this->method_description = __('Online GLS Shipping Method for courier', 'gls');
            ...
            {
                $help_tip = wc_help_tip('In cazul in care intampinati probleme la configurare va rugam sa verificati documentatia CurieRO - GLS Livrare: Ghid de completare a setărilor de livrare.');
                echo <<<HTML
                    ...
                    <h2>CurieRO - Metoda de livrare GLS {$help_tip}</h2>
            ...
            public function calculate_shipping($package = []): void
            {
                ...
                $args = apply_filters('curiero_overwrite_gls_shipping', $args, $judetdest, $orasdest);
                $this->add_rate($args);
            }
        }
    }
...
add_action('admin_menu', function (): void {
    add_submenu_page(
        ...
        'gls_redirect',
        function (): void {
            wp_safe_redirect(curiero_build_url('admin.php?page=wc-settings&tab=shipping&section=gls'));
            exit;
        }
    );
});
