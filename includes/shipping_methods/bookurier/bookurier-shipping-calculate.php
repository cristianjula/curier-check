...
function Bookurier_Shipping_Method(): void
{
    if (!class_exists('Bookurier_Shipping_Method')) {
        class Bookurier_Shipping_Method extends WC_Shipping_Method
        {
            ...
            {
                $this->id = 'bookurier';
                $this->method_title = __('Bookurier Shipping', 'curiero-plugin');
                $this->method_description = __('Bookurier Shipping Method for courier', 'curiero-plugin');
            ...
            public function init_form_fields(): void
            {
                $this->form_fields = [
                    'title' => [
                        ...
                        'default' => __('Bookurier', 'curiero-plugin'),
                        'desc_tip' => true,
            ...
            {
                $help_tip = wc_help_tip('In cazul in care intampinati probleme la configurare va rugam sa verificati documentatia CurieRO - Bookurier Livrare: Ghid de completare a setărilor de livrare.');
                echo <<<HTML
                    ...
                    <h2>CurieRO - Metoda de livrare Bookurier {$help_tip}</h2>
            ...
            public function calculate_shipping($package = []): void
            {
                ...
                $args = apply_filters('curiero_overwrite_bookurier_shipping', $args, $judetdest, $orasdest);
                $this->add_rate($args);
            }
        }
    }
...
add_action('admin_menu', function (): void {
    add_submenu_page(
        ...
        'bookurier_redirect',
        function (): void {
            wp_safe_redirect(curiero_build_url('admin.php?page=wc-settings&tab=shipping&section=bookurier'));
            exit;
        }
    );
});
