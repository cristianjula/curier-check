...
function Memex_Shipping_Method(): void
{
    if (!class_exists('Memex_Shipping_Method')) {
        class Memex_Shipping_Method extends WC_Shipping_Method
        {
            ...
            {
                $this->id = 'memex';
                $this->method_title = __('PTT Express Shipping', 'curiero-plugin');
                $this->method_description = __('PTT Express Shipping Method for courier', 'curiero-plugin');
            ...
            public function init_form_fields(): void
            {
                $this->form_fields = [
                    'title' => [
                        ...
                        'default' => __('PTT Express', 'curiero-plugin'),
                        'desc_tip' => true,
            ...
            {
                $help_tip = wc_help_tip('In cazul in care intampinati probleme la configurare va rugam sa verificati documentatia CurieRO - PTT Express Livrare: Ghid de completare a setărilor de calcul cost in checkout.');
                echo <<<HTML
                    ...
                    <h2>CurieRO - Metoda de livrare PTT Express {$help_tip}</h2>
            ...
            public function calculate_shipping($package = []): void
            {
                ...
                $args = apply_filters('curiero_overwrite_memex_shipping', $args, $judetdest, $orasdest);
                $this->add_rate($args);
            }
        }
    }
...
add_action('admin_menu', function (): void {
    add_submenu_page(
        ...
        'ptt_redirect',
        function (): void {
            wp_safe_redirect(curiero_build_url('admin.php?page=wc-settings&tab=shipping&section=memex'));
            exit;
        }
    );
});
