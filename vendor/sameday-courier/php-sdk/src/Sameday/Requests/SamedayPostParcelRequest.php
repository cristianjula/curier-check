...
class SamedayPostParcelRequest implements SamedayRequestInterface
{
     ...
     * @param bool $last
     */
    public function __construct($awbNumber, ParcelDimensionsObject $parcelDimensions, $position, $observation = null, $priceObservation = null, $last = \false)
    {
    ...
    {
        return new SamedayRequest(\true, 'POST', "/api/awb/{$this->awbNumber}/parcel", [], new RequestBodyUrlEncoded(['weight' => $this->parcelDimensions->getWeight(), 'width' => $this->parcelDimensions->getWidth(), 'length' => $this->parcelDimensions->getLength(), 'height' => $this->parcelDimensions->getHeight(), 'position' => $this->position, 'isLast' => $this->last ? 1 : 0, 'observation' => $this->observation, 'priceObservation' => $this->priceObservation]));
    }
    /**
     * @return string
     */
