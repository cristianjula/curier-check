...
use CurieRO\Sameday\Objects\ParcelDimensionsObject;
use CurieRO\Sameday\Objects\PostAwb\Request\AwbRecipientEntityObject;
use CurieRO\Sameday\Objects\PostAwb\Request\ThirdPartyPickupEntityObject;
use CurieRO\Sameday\Objects\Types\AwbPaymentType;
use CurieRO\Sameday\Objects\Types\CodCollectorType;
use CurieRO\Sameday\Objects\Types\DeliveryIntervalServiceType;
...
class SamedayPostAwbRequest implements SamedayRequestInterface
{
     ...
     */
    public function __construct($pickupPointId, $contactPersonId, PackageType $packageType, array $parcelsDimensions, $serviceId, AwbPaymentType $awbPayment, AwbRecipientEntityObject $awbRecipient, $insuredValue, $cashOnDeliveryAmount = 0.0, CodCollectorType $cashOnDeliveryCollector = null, ThirdPartyPickupEntityObject $thirdPartyPickup = null, array $serviceTaxIds = [], DeliveryIntervalServiceType $deliveryIntervalServiceType = null, $reference = null, $observation = null, $priceObservation = null, $clientObservation = null, $lockerFirstMile = null, $lockerLastMile = null)
    {
    ...
    public function buildRequest()
    {
        ...
        $weight = 0;
        array_map(static function (ParcelDimensionsObject $parcelDimensions) use (&$weight) {
            $weight += $parcelDimensions->getWeight();
        }, $this->parcelsDimensions);
        $body = ['pickupPoint' => $this->pickupPointId, 'contactPerson' => $this->contactPersonId, 'packageType' => $this->packageType->getType(), 'packageNumber' => count($this->parcelsDimensions), 'packageWeight' => $weight, 'service' => $this->serviceId, 'awbPayment' => $this->awbPayment->getType(), 'cashOnDelivery' => $this->cashOnDeliveryAmount, 'cashOnDeliveryReturns' => $this->cashOnDeliveryCollector ? $this->cashOnDeliveryCollector->getType() : null, 'insuredValue' => $this->insuredValue, 'thirdPartyPickup' => $this->thirdPartyPickup ? 1 : 0];
        // Third party pickup.
        if ($this->thirdPartyPickup !== null) {
            $body = array_merge($body, ['thirdParty' => $this->thirdPartyPickup->getFields()]);
        }
        $body = array_merge($body, ['serviceTaxes' => $this->serviceTaxIds, 'deliveryInterval' => $this->deliveryIntervalServiceType ? $this->deliveryIntervalServiceType->getType() : null, 'awbRecipient' => $this->awbRecipient->getFields(), 'parcels' => array_map(
            // Build parcel fields from ParcelDimensionsObject.
            static function (ParcelDimensionsObject $parcelDimensions) {
                return ['weight' => $parcelDimensions->getWeight(), 'width' => $parcelDimensions->getWidth(), 'length' => $parcelDimensions->getLength(), 'height' => $parcelDimensions->getHeight()];
            },
            $this->parcelsDimensions
        ), 'observation' => $this->observation, 'priceObservation' => $this->priceObservation, 'clientInternalReference' => $this->reference, 'clientObservation' => $this->clientObservation]);
        ...
        return new SamedayRequest(\true, 'POST', '/api/awb', [], new RequestBodyUrlEncoded($body));
    }
    /**
     * @return int
     */
