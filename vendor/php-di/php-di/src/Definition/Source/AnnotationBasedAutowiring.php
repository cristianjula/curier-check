...
use CurieRO\DI\Annotation\Inject;
use CurieRO\DI\Annotation\Injectable;
use CurieRO\DI\Definition\Exception\InvalidAnnotation;
use CurieRO\DI\Definition\ObjectDefinition;
use CurieRO\DI\Definition\ObjectDefinition\MethodInjection;
use CurieRO\DI\Definition\ObjectDefinition\PropertyInjection;
use CurieRO\DI\Definition\Reference;
use CurieRO\Doctrine\Common\Annotations\AnnotationRegistry;
use CurieRO\Doctrine\Common\Annotations\Reader;
use CurieRO\Doctrine\Common\Annotations\SimpleAnnotationReader;
use InvalidArgumentException;
use CurieRO\PhpDocReader\PhpDocReader;
use ReflectionClass;
...
class AnnotationBasedAutowiring implements DefinitionSource, Autowiring
{
    ...
    private function readProperty(ReflectionProperty $property, ObjectDefinition $definition, $classname = null)
    {
        ...
        $annotation = $this->getAnnotationReader()->getPropertyAnnotation($property, 'CurieRO\DI\Annotation\Inject');
    ...
    private function getMethodInjection(ReflectionMethod $method)
    {
        // Look for @Inject annotation
        try {
            $annotation = $this->getAnnotationReader()->getMethodAnnotation($method, 'CurieRO\DI\Annotation\Inject');
    ...
    public function getAnnotationReader()
    {
        if ($this->annotationReader === null) {
            ...
            $this->annotationReader->addNamespace('CurieRO\DI\Annotation');
        }
    ...
    private function readInjectableAnnotation(ReflectionClass $class, ObjectDefinition $definition)
    {
        try {
            ...
            $annotation = $this->getAnnotationReader()->getClassAnnotation($class, 'CurieRO\DI\Annotation\Injectable');
