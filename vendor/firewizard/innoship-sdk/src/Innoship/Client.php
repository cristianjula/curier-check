...
use CurieRO\Innoship\Response\ResponseFactory;
class Client
{
    private $apiKey;
    private $version = '1.0';
    ...
    public function request($path, $method, array $data = null): Contract
    {
        ...
        curl_setopt($this->curl, \CURLOPT_FOLLOWLOCATION, \true);
        $headers = ["X-Api-Key: {$this->apiKey}", "api-version: {$this->version}", 'Content-type: application/json; charset=utf-8', 'Accept: application/json,application/pdf'];
        curl_setopt($this->curl, \CURLOPT_HTTPHEADER, $headers);
        if ($method == 'POST') {
            curl_setopt($this->curl, \CURLOPT_POST, \true);
            if ($data) {
