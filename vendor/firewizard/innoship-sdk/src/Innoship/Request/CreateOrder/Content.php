...
class Content implements Contract
{
    ...
    {
        return array_filter(['EnvelopeCount' => $this->envelopeCount, 'ParcelsCount' => $this->parcelsCount, 'PalettesCount' => $this->palettesCount, 'TotalWeight' => $this->totalWeight, 'Contents' => $this->contents, 'Package' => $this->package, 'OversizedPackage' => $this->oversizedPackage, 'Parcels' => is_array($this->parcels) ? array_map(function (Parcel $parcel) {
            return $parcel->data();
        }, $this->parcels) : null], function ($el) {
            return $el !== null;
        });
    }
